<%@ page import="basicDatas.classDatas" %><%--
...
<%-------------------------------%>
<%
    ArrayList<classDatas> classDatasArrayList = (ArrayList<classDatas>)request.getAttribute("classesDatas");
    ArrayList<classDatas> classNameList = (ArrayList<classDatas>)request.getSession().getAttribute("classesNameList");
    ArrayList<majorDatas> majorNameListForclass = (ArrayList<majorDatas>)request.getSession().getAttribute("majorNameListForclass");
    String classMessage = (String)request.getAttribute("message");
...
<div id="classMessageBox" style="width: 100px; padding: 3px; position: fixed; top: 2%; left: 50%; right: 50%;
    ...
    <h6 style="text-align: center;"><%=classMessage%></h6>
</div>
<script type="text/javascript">
    setTimeout(function(){document.getElementById("classMessageBox").style.display="none";},3000);
</script>
...
<div id="alert_insert" class="row register-form" style="margin:auto; top:0; left: 0; right: 0; bottom: 0;
  display: none; position: fixed; z-index: 10000; background-color: rgba(191,191,191,.9)">
    <div class="col-md-8 offset-md-2">
        <form action="classPage" method="post" onsubmit="return check()" class="custom-form" style="width: 700px; padding: 30px; position: fixed; top: 15%; left: 27%; right: 27%;
        background-color: rgb(255,255,255); box-shadow: 0 0.1rem 1rem rgba(0,0,0,.1)!important; border: 1px solid rgba(0,0,0,.125); border-radius: .25rem;">
            ...
            %>
            <div class="form-row form-group">
                <div class="col-sm-4 label-column"><label id="insertLabel+2" class="col-form-label" for="name-input-field">所属专业</label></div>
                <div class="col-sm-6 input-column">
                    <select id="insert+2" name="insert+2" class="form-control form-control-sm custom-select custom-select-sm">
                        ...
                        <% for(int i=0;i<majorNameListForclass.size();++i)
                        { %>
                        <option value="<%= majorNameListForclass.get(i).getMajorNum()%>"><%= majorNameListForclass.get(i).getMajorName()%></option>
                        <% }%>
                    </select>&nbsp;
                </div>
            </div>
...
<div id="alert_update" class="row register-form" style="margin:auto; top:0; left: 0; right: 0; bottom: 0;
  display: none; position: fixed; z-index: 10000; background-color: rgba(191,191,191,.9)">
    <div class="col-md-8 offset-md-2">
        <form action="classPage" method="post" class="custom-form" style="width: 700px; padding: 30px; position: fixed; top: 15%; left: 27%; right: 27%;
        background-color: rgb(255,255,255); box-shadow: 0 0.1rem 1rem rgba(0,0,0,.1)!important; border: 1px solid rgba(0,0,0,.125); border-radius: .25rem;">
            <input name="operationNumber" type="hidden" value="3">
            <h1>更新数据</h1>
            <input name="classNumUpdate" type="hidden" >
            <%
                for(int i=0; i<2; ++i)
                {
            %>
            ...
            %>
            <div class="form-row form-group">
                <div class="col-sm-4 label-column"><label id="updateLabel+2" class="col-form-label" for="name-input-field">所属专业</label></div>
                <div class="col-sm-6 input-column">
...
<script>
    ...
    function update_onclick(getvalue, listNum, formTitle)
    {
        ...
        document.getElementsByName("classNumUpdate")[0].value = getvalue;
    }
</script>
<%-------------------------------%>
  ...
  display: none; position: fixed; z-index: 10000; background-color: rgba(191,191,191,.9)">
    <div class="col-md-8 offset-md-2">
        ...
        background-color: rgb(255,255,255); box-shadow: 0 0.1rem 1rem rgba(0,0,0,.1)!important; border: 1px solid rgba(0,0,0,.125); border-radius: .25rem;">
            ...
            <h1>删除数据</h1>
            <input name="classNumDelete" type="hidden" >
            <div class="form-check">
...
<script>
    ...
    function delete_onclick(getvalue)
    {
        ...
        document.getElementsByName("classNumDelete")[0].value = getvalue;
    }
</script>
<%-------------------------------%>
<div class="container-fluid" id="table_class" style="margin-top: 20px" >
    ...
    <div class="card shadow">
        <div class="card-header py-3">
            <p class="text-primary m-0 font-weight-bold">班级信息</p>
        </div>
        <div class="card-body">
            <form action="classPage" method="post">
            <div class="row">
                <div class="col-md-6 text-nowrap">
                    <div id="dataTable_length" class="dataTables_length" aria-controls="dataTable">
                        ...
                        <label style="margin-right: 40px">班级
                            <select name="classSelect" class="form-control form-control-sm custom-select custom-select-sm">
                                <option value="pleaseSelect" selected="selected">请选择</option>
                                <% for(int i=0;i<classNameList.size();++i)
                                { %>
                                <option value="<%= classNameList.get(i).getClassNum()%>"><%= classNameList.get(i).getClassName()%></option>
                                <% }%>
                            </select>&nbsp;
                        </label>
                    </div>
            ...
            <div class="table-responsive table mt-2" id="dataTable" role="grid" aria-describedby="dataTable_info">
                <table class="table my-0" id="dataTable">
                    ...
                    <tbody>
                    <%
                        if(classDatasArrayList!=null)
                        {
                            for(int i=0; i<classDatasArrayList.size(); ++i)
                            {
                                classDatas coD = classDatasArrayList.get(i);

                    %>
                    <tr id="<%= coD.getClassNum()%>">
                        <td><%= coD.getClassName()%></td>
                        <td><%= coD.getClassLeader()%></td>
                        <td><%= coD.getMajorBelonging()%></td>
                        <td><%= coD.getCollegeBelonging()%></td>
                        <% if(lid.getRole().equals("系统")
                        ) {%>
                        <td><a href="javascript:void(0)" onclick="update_onclick('<%= coD.getClassNum()%>', 2, 'form_title')">更新</a>&nbsp;&nbsp;
                            <a href="javascript:void(0)" onclick="delete_onclick('<%= coD.getClassNum()%>')">删除</a></td>
                        <% } %>
                    </tr>
                    <%      }
                    }
